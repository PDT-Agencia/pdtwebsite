<nav class="navbar navbar-expand-lg navbar-light container pt-4">
    <div class="container-fluid">
        <a class="navbar-brand" href="{{ route('home') }}">
            <svg id="logoLink" xmlns="http://www.w3.org/2000/svg" width="96" height="38" viewBox="0 0 96 38" fill="none">
                <path
                    d="M39.6763 22.0728C38.0719 26.8796 34.3995 30.2069 31.9465 28.4814V31.631C33.4119 31.4855 35.0845 30.9408 36.5151 29.5012C37.043 28.8239 37.5903 28.0707 38.1041 27.2788C38.1041 27.2762 38.1066 27.2749 38.1066 27.2723C39.1393 25.6769 40.0291 23.9219 40.3175 22.3007C41.2202 19.2477 40.472 16.3273 40.472 16.3273C40.5609 19.0662 40.0754 21.0221 39.7085 22.0857C39.6969 22.0805 39.6866 22.0767 39.675 22.0715L39.6763 22.0728Z"
                    fill="white" />
                <path
                    d="M58.4901 3.44832C56.9694 2.11818 55.2761 1.21039 53.409 0.726232C51.5432 0.242077 49.3053 0 46.6952 0H36.6645C35.006 0 33.8046 0.37213 33.0616 1.1151C32.3187 1.85807 31.9465 3.05945 31.9465 4.71794V14.2774C33.8497 11.1948 36.3258 9.17577 36.9323 8.70578C38.1105 7.525 39.6582 6.6919 41.4313 6.4099C45.1951 5.81243 48.7658 7.88555 50.2131 11.227L56.7118 10.1956C57.6428 10.0346 58.4051 10.0153 59.0309 10.0848C59.3026 9.98055 59.6013 9.92131 59.9168 9.92131C61.1297 9.92131 62.3594 9.85564 62.1122 11.8309C62.08 12.0871 62.0053 12.3318 61.892 12.5546C62.9222 28.9888 48.9847 22.8943 46.542 21.7033C46.3875 21.7857 46.2291 21.8629 46.0682 21.9351C45.7707 23.1055 44.9994 26.559 45.3522 29.7588C45.6548 31.81 46.5626 34.7227 49.0375 37.9302C49.9324 37.8748 50.7732 37.786 51.5548 37.6585C52.9455 37.4344 54.246 37.0456 55.4564 36.4919C56.6655 35.9395 57.7806 35.2042 58.8004 34.2887C60.0971 33.0964 61.1594 31.7443 61.9886 30.2326C62.8179 28.7209 63.4308 27.0225 63.8287 25.1387C64.2265 23.2549 64.4248 21.1728 64.4248 18.8923C64.4248 11.9803 62.4457 6.83096 58.4888 3.44317L58.4901 3.44832Z"
                    fill="white" />
                <path
                    d="M48.1695 12.0137C48.4489 11.97 48.5945 11.4227 48.494 10.7918C48.3936 10.1608 48.0858 9.68567 47.8064 9.73074C47.527 9.77452 47.3815 10.3231 47.4819 10.9527C47.5824 11.5837 47.8901 12.0588 48.1695 12.0137Z"
                    fill="white" />
                <path
                    d="M0 3.78975C0 2.00636 1.40482 0.0620117 3.67365 0.0620117H16.1535C23.1763 0.0620117 29.4987 4.76192 29.4987 13.7845C29.4987 22.3203 23.1776 27.0755 16.1535 27.0755H7.13098V34.2065C7.13098 36.5835 5.618 37.9343 3.67365 37.9343C1.89026 37.9343 0 36.5835 0 34.2065V3.78975ZM7.13098 6.8698V20.3231H16.1535C19.7731 20.3231 22.6368 17.1349 22.6368 13.7857C22.6368 10.0039 19.7731 6.8698 16.1535 6.8698H7.13098Z"
                    fill="white" />
                <path
                    d="M75.8514 7.00862H66.3537C61.6319 7.00862 61.6319 0.0617676 66.3537 0.0617676H92.0255C96.8555 0.0617676 96.8555 7.00862 92.0255 7.00862H83.0159V34.4175C83.0159 39.1393 75.8514 39.2487 75.8514 34.4175V7.00862Z"
                    fill="white" />
            </svg>

            <img id='logo' src="{{asset('image/logo-color.png')}}" alt="">
        </a>
        <button class="navbar-toggler tex-white" type="button" data-bs-toggle="collapse"
            data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false"
            aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">

            </ul>
            <ul class="nav-ul-flex navbar-nav mb-2 mb-lg-0">
                <li class="btn mx-1 text-white  my-2 my-sm-0">
                    <a href="{{ route('service') }}">{{ __('Our Services') }}</a>
                </li>
                <li class="btn mx-1 text-white  my-2 my-sm-0">
                    <a href="{{ route('portfolio') }}">{{ __('Portfolio') }}</a>
                </li>
                <li class="btn mx-1 text-white  my-2 my-sm-0">
                    <a href="{{ route('about') }}">{{ __('About Us') }}</a>
                </li>
                <li class="btn mx-1 text-white  my-2 my-sm-0">
                    <a href="#">Blog</a>
                </li>
                <li class="button-cont hover:text-white my-2 btn fill mx-1 my-sm-0 nav-contact">
                 <a href="{{ route('contact') }}">{{ __('Contact') }}</a>
                </li>
                <li class="my-2 my-sm-0">

                    <div class="dropdown">
                        <a class="btn" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown"
                            aria-expanded="false">{{ __('Language') }}
                            <svg xmlns="http://www.w3.org/2000/svg" width="13" height="8" viewBox="0 0 13 8"
                                fill="none">
                                <path
                                    d="M12.4102 0.940948C12.4142 0.757525 12.361 0.577404 12.258 0.425624C12.1549 0.273844 12.007 0.157947 11.8351 0.0940481C11.6631 0.030149 11.4755 0.021422 11.2983 0.0690802C11.1212 0.116738 10.9633 0.218413 10.8466 0.35997L6.21948 5.76116L1.59399 0.35997C1.52202 0.262048 1.43065 0.180041 1.32552 0.119084C1.22039 0.058127 1.10377 0.0195338 0.983032 0.00572064C0.862295 -0.00809253 0.740032 0.00317224 0.62385 0.0388094C0.507668 0.0744465 0.400061 0.133688 0.307835 0.212825C0.215608 0.291961 0.140797 0.389288 0.0879268 0.498709C0.0350561 0.608131 0.00536153 0.72729 0.000676118 0.848725C-0.00400929 0.970159 0.0163602 1.09125 0.0606422 1.20442C0.104923 1.31759 0.172093 1.4204 0.257948 1.50641L5.54886 7.69028C5.63174 7.78739 5.7346 7.86537 5.85053 7.91884C5.96645 7.97231 6.09263 8 6.2203 8C6.34796 8 6.47414 7.97231 6.59007 7.91884C6.70599 7.86537 6.80896 7.78739 6.89183 7.69028L12.1878 1.50641C12.3266 1.3502 12.4053 1.14983 12.4102 0.940948Z"
                                    fill="white" />
                            </svg>
                        </a>
                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                            <li>
                                <form method="GET" action="{{ route('cambiar-locale', ['locale' => 'es']) }}">
                                    @csrf
                                    <button class="dropdown-item" type="submit">Español</button>
                                </form>
                            </li>
                            <li>
                                <form method="GET" action="{{ route('cambiar-locale', ['locale' => 'en']) }}">
                                    @csrf
                                    <button class="dropdown-item" type="submit">Ingles</button>
                                </form>
                            </li>

                        </ul>
                    </div>

                </li>
            </ul>
        </div>
    </div>
</nav>


{{-- <a class="navbar-brand" href="{{ route('home') }}">
        <svg xmlns="http://www.w3.org/2000/svg" width="96" height="38" viewBox="0 0 96 38" fill="none">
            <path
                d="M39.6763 22.0728C38.0719 26.8796 34.3995 30.2069 31.9465 28.4814V31.631C33.4119 31.4855 35.0845 30.9408 36.5151 29.5012C37.043 28.8239 37.5903 28.0707 38.1041 27.2788C38.1041 27.2762 38.1066 27.2749 38.1066 27.2723C39.1393 25.6769 40.0291 23.9219 40.3175 22.3007C41.2202 19.2477 40.472 16.3273 40.472 16.3273C40.5609 19.0662 40.0754 21.0221 39.7085 22.0857C39.6969 22.0805 39.6866 22.0767 39.675 22.0715L39.6763 22.0728Z"
                fill="white" />
            <path
                d="M58.4901 3.44832C56.9694 2.11818 55.2761 1.21039 53.409 0.726232C51.5432 0.242077 49.3053 0 46.6952 0H36.6645C35.006 0 33.8046 0.37213 33.0616 1.1151C32.3187 1.85807 31.9465 3.05945 31.9465 4.71794V14.2774C33.8497 11.1948 36.3258 9.17577 36.9323 8.70578C38.1105 7.525 39.6582 6.6919 41.4313 6.4099C45.1951 5.81243 48.7658 7.88555 50.2131 11.227L56.7118 10.1956C57.6428 10.0346 58.4051 10.0153 59.0309 10.0848C59.3026 9.98055 59.6013 9.92131 59.9168 9.92131C61.1297 9.92131 62.3594 9.85564 62.1122 11.8309C62.08 12.0871 62.0053 12.3318 61.892 12.5546C62.9222 28.9888 48.9847 22.8943 46.542 21.7033C46.3875 21.7857 46.2291 21.8629 46.0682 21.9351C45.7707 23.1055 44.9994 26.559 45.3522 29.7588C45.6548 31.81 46.5626 34.7227 49.0375 37.9302C49.9324 37.8748 50.7732 37.786 51.5548 37.6585C52.9455 37.4344 54.246 37.0456 55.4564 36.4919C56.6655 35.9395 57.7806 35.2042 58.8004 34.2887C60.0971 33.0964 61.1594 31.7443 61.9886 30.2326C62.8179 28.7209 63.4308 27.0225 63.8287 25.1387C64.2265 23.2549 64.4248 21.1728 64.4248 18.8923C64.4248 11.9803 62.4457 6.83096 58.4888 3.44317L58.4901 3.44832Z"
                fill="white" />
            <path
                d="M48.1695 12.0137C48.4489 11.97 48.5945 11.4227 48.494 10.7918C48.3936 10.1608 48.0858 9.68567 47.8064 9.73074C47.527 9.77452 47.3815 10.3231 47.4819 10.9527C47.5824 11.5837 47.8901 12.0588 48.1695 12.0137Z"
                fill="white" />
            <path
                d="M0 3.78975C0 2.00636 1.40482 0.0620117 3.67365 0.0620117H16.1535C23.1763 0.0620117 29.4987 4.76192 29.4987 13.7845C29.4987 22.3203 23.1776 27.0755 16.1535 27.0755H7.13098V34.2065C7.13098 36.5835 5.618 37.9343 3.67365 37.9343C1.89026 37.9343 0 36.5835 0 34.2065V3.78975ZM7.13098 6.8698V20.3231H16.1535C19.7731 20.3231 22.6368 17.1349 22.6368 13.7857C22.6368 10.0039 19.7731 6.8698 16.1535 6.8698H7.13098Z"
                fill="white" />
            <path
                d="M75.8514 7.00862H66.3537C61.6319 7.00862 61.6319 0.0617676 66.3537 0.0617676H92.0255C96.8555 0.0617676 96.8555 7.00862 92.0255 7.00862H83.0159V34.4175C83.0159 39.1393 75.8514 39.2487 75.8514 34.4175V7.00862Z"
                fill="white" />
        </svg>
    </a>
 

    <div >
        <div class="btn mx-4 text-white  my-2 my-sm-0">
            <a href="#">{{ __('Our Services') }}</a>
        </div>
        <div class="btn mx-4 text-white  my-2 my-sm-0">
            <a href="#">{{ __('Portfolio') }}</a>
        </div>
        <div class="btn mx-4 text-white  my-2 my-sm-0">
            <a href="{{ route('about') }}">{{ __('About Us') }}</a>
        </div>
        <div class="btn mx-4 text-white  my-2 my-sm-0">
            <a href="#">Blog</a>
        </div>
        <div class="button-cont hover:text-white my-2 btn fill mx-4 my-sm-0 nav-contact">
            <a href="#">{{ __('Contact') }}</a>
        </div>
        <div class="btn  my-2 my-sm-0">
            <a data-toggle="collapse" href="#collapseExample" role="button" aria-expanded="false"
                aria-controls="collapseExample">{{ __('Language') }}</a>
            <div class="collapse" id="collapseExample">
                <div class="card card-body">
                    Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid.
                    Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident.
                </div>
            </div>
            <svg xmlns="http://www.w3.org/2000/svg" width="13" height="8" viewBox="0 0 13 8" fill="none">
                <path
                    d="M12.4102 0.940948C12.4142 0.757525 12.361 0.577404 12.258 0.425624C12.1549 0.273844 12.007 0.157947 11.8351 0.0940481C11.6631 0.030149 11.4755 0.021422 11.2983 0.0690802C11.1212 0.116738 10.9633 0.218413 10.8466 0.35997L6.21948 5.76116L1.59399 0.35997C1.52202 0.262048 1.43065 0.180041 1.32552 0.119084C1.22039 0.058127 1.10377 0.0195338 0.983032 0.00572064C0.862295 -0.00809253 0.740032 0.00317224 0.62385 0.0388094C0.507668 0.0744465 0.400061 0.133688 0.307835 0.212825C0.215608 0.291961 0.140797 0.389288 0.0879268 0.498709C0.0350561 0.608131 0.00536153 0.72729 0.000676118 0.848725C-0.00400929 0.970159 0.0163602 1.09125 0.0606422 1.20442C0.104923 1.31759 0.172093 1.4204 0.257948 1.50641L5.54886 7.69028C5.63174 7.78739 5.7346 7.86537 5.85053 7.91884C5.96645 7.97231 6.09263 8 6.2203 8C6.34796 8 6.47414 7.97231 6.59007 7.91884C6.70599 7.86537 6.80896 7.78739 6.89183 7.69028L12.1878 1.50641C12.3266 1.3502 12.4053 1.14983 12.4102 0.940948Z"
                    fill="white" />
            </svg>
        </div>
    </div> --}}
